...
colnames(inds) <- c("body_mass", "births", "dob", "deaths")
...
birth <- function(inds, lambda = 0.5, repr_col = 2){
    total_inds       <- dim(inds)[1] # Number of individuals (rows) in inds
    ind_cols         <- dim(inds)[2] # Number of states (columns) recorded in inds
    inds[, repr_col] <- rpois(n = total_inds, lambda = lambda) # number offspring per individual
    total_off        <- sum(inds[, repr_col]) # sum offspring across individuals 
    
    # ---- We now have the total number of new offspring; now add to inds
    new_inds     <- array(data = 0, dim = c(total_off, ind_cols)) # empty array with updated dims
    new_inds[,1] <- rnorm(n = dim(new_inds)[1], mean = 23, sd = 3) # fill in col1 with body mass
...
## Death function
```{r}
death <- function(inds, d = 0.1, death_col = 4){
    total_inds       <- dim(inds)[1] # Get the number of individuals in inds
    ind_cols         <- dim(inds)[2] # Total inds columns
    inds[, death_col] <- rbinom(n = total_inds, size = 1, prob = 0.1) # 10% die
    total_dead        <- sum(inds[, death_col])
    
    # ---- We now have the total number of dead offspring
    dead_inds     <- array(data = 0, dim = c(total_dead, ind_cols))
    #new_inds[,1] <- rnorm(n = dim(new_inds)[1], mean = 23, sd = 3)
    
    # ---- Our new offspring can now be attached in the inds array
    inds <- rbind(inds, dead_inds)
    return(inds)
}
```
...
while(ts < time_steps){
    inds            <- birth(inds)
    inds            <- death(inds)
    inds            <- inds[inds[, 4] == 0,] # Retain living
    ts              <- ts + 1 
    inds_hist[[ts]] <- inds
}

ind_abund <- array(data = NA, dim = c(time_steps, 2))
for(i in 1:10){
...
colnames(inds) <- c("body_mass", "births", "deaths", "repr", "death")
rownames(inds) <- c("ind_1", "ind_2", "ind_3", "ind_4", "ind_5")
